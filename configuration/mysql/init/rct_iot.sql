...
CREATE TABLE `beacons_tbl` (
  ...
  `inallot` int(11) DEFAULT NULL COMMENT '分配状态：Yes,No',
...
CREATE TABLE `buildings_tbl` (
  ...
  `buildtype` int(11) DEFAULT NULL COMMENT '类型：2D,3D',
  `flag` tinyint(1) DEFAULT '0' COMMENT '整体展示(false/true)',
...
CREATE TABLE `customer_branch_tbl` (
  ...
  `type` int(11) DEFAULT NULL COMMENT 'worktype,depart,assetype,tboxtype',
...
CREATE TABLE `customer_members_tbl` (
  ...
  `sex` int(11) DEFAULT NULL COMMENT '男,女,未知',
...
CREATE TABLE `dev_config_v2` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `deveui` varchar(16) NOT NULL,
  `TYPE` int(11) DEFAULT NULL COMMENT '工作模式：tracker,gateway',
  `POWER` int(11) DEFAULT NULL COMMENT '发射功率：20,18,17,16,15,14,13,12,11,10',
  `DR` int(11) DEFAULT NULL COMMENT 'DR5,DR4,DR3,DR2,DR1',
  `MODE` int(11) DEFAULT NULL COMMENT '当前模式：CN470,US915,EU868,AS923,IN865,RU864,AU915,KR920',
  `ALREPORT` int(11) DEFAULT NULL COMMENT '自动上报模式：disabled,enabled',
  `BLE` int(11) DEFAULT NULL COMMENT '蓝牙扫描周期：disabled,5s,10s,20s,30s,1m,2m,5m,10m,15m,20m,30m,1h,2h,6h,12h',
  `SCAN` int(11) DEFAULT NULL COMMENT '蓝牙扫描持续时间：1s,2s,3s,6s,9s,12s,15s,Always',
  `SCALE` int(11) DEFAULT NULL COMMENT '指示扫描信标的类型：position/hazardous,position/asset/hazardous,proximity badge,all,asset',
  `STEPSOFF` tinyint(4) DEFAULT '0' COMMENT '步数门限',
  `BLEOFF` tinyint(4) DEFAULT '0' COMMENT '蓝牙门限',
  `BUZZER` int(11) DEFAULT NULL COMMENT '蜂鸣器：disable,enable',
  `VIBRATOR` int(11) DEFAULT NULL COMMENT '震动：disable,enable',
  `DISTANCE` int(11) DEFAULT NULL COMMENT '解除告警距离：2m,4m,6m,8m,10m,15m,unlimit',
  `PROXIMITY` int(11) DEFAULT NULL COMMENT '近距离接触触发告警：disabled,enabled',
  `GPS` int(11) DEFAULT NULL COMMENT 'GPS上报周期：disabled,5s,10s,15s,30s,60s,150s,5m,15m,30m,60m,90m,3h,6h',
  `HB` int(11) DEFAULT NULL COMMENT '心跳周期：1min,5min,10min,20min,30min,1h,2h,6h,12h,24h',
...
CREATE TABLE `dev_list_tbl` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `joinmode` int(11) DEFAULT NULL COMMENT '入网模式：OTAA,ABP,Other',
  `deveui` varchar(16) NOT NULL COMMENT '设备号',
  `scheme` int(11) DEFAULT NULL COMMENT '制式：CN470,US915_BAND1,US915_BAND2,EU868,AS923,IN865,RU864,AU915_BAND1,AU915_BAND2,KR920',
  `devtype` int(11) DEFAULT NULL COMMENT '硬件设备类型：badge,gateway,tbox',
  `type` int(11) DEFAULT NULL COMMENT '当前工作模式：tracker,gateway',
  `nettype` int(11) DEFAULT '0' COMMENT '网络类型:0-lora,1-wifi4g',
  ...
  `alias` varchar(20) DEFAULT NULL COMMENT '设备别名',
  `custom` int(11) DEFAULT NULL COMMENT '系统设备：Yes,No',
  `inallot` int(11) DEFAULT NULL COMMENT '分配状态：Yes,No',
  `inuse` int(11) DEFAULT NULL COMMENT '使用状态：Yes,No',
  `workstate` int(11) DEFAULT NULL COMMENT '工作状态：正常,故障',
  `hbstatus` int(11) DEFAULT NULL COMMENT '在线状态：online,offline,unknown',
  `clockin` int(11) DEFAULT NULL COMMENT '打卡点：Yes,No',
  ...
  `groundid` int(11) unsigned DEFAULT NULL COMMENT '楼层编号',
  `tranche` varchar(32) DEFAULT NULL COMMENT '分组名称',
  `gpstype` int(11) DEFAULT NULL COMMENT '最后一次位置的定位类型：Ble,Gps',
  `anglimit` tinyint(4) NOT NULL DEFAULT '15' COMMENT '单点定位时，限制定位的角度',
  ...
  `postype` int(11) DEFAULT NULL COMMENT '定位算法类型：三点定位,区域定位,GPS定位,网关,四点定位',
  `keyenc` int(11) DEFAULT NULL COMMENT '加密存储：Yes,No',
  ...
  `workmode` int(11) DEFAULT NULL COMMENT '设备当前状态：Normal,Test,Sold',
  `debugmode` int(11) DEFAULT NULL COMMENT '设备调试状态：Debug,Info,Error',
  `configmode` int(11) DEFAULT NULL COMMENT '设备配置模式：Yes,No',
...
CREATE TABLE `dev_other_tbl` (
  ...
  `deveui` varchar(16) NOT NULL COMMENT '设备号',
  `alias` varchar(20) DEFAULT NULL COMMENT '设备别名',
  `usetype` int(11) DEFAULT NULL COMMENT '使用类型：布置,绑定',
  `devtype` int(11) DEFAULT NULL COMMENT '设备类型：资产网关,AOA网关,烟感,报警器,蓝牙传感器,摄像头',
  `custom` int(11) DEFAULT NULL COMMENT '设备别名',
...
CREATE TABLE `dev_tranche_tbl` (
  ...
  `projectid` char(8) NOT NULL DEFAULT '' COMMENT '项目编号',
  `fremove` int(11) DEFAULT NULL COMMENT '区域防抖：Yes,No',
  PRIMARY KEY (`id`),
...
CREATE TABLE `fence_manage_tbl` (
  ...
  `flag` tinyint(1) NOT NULL DEFAULT '1' COMMENT '开启状态(false/true)',
...
CREATE TABLE `map_tbl` (
  ...
  `scale` int(11) unsigned NOT NULL DEFAULT '1' COMMENT '地图缩放比例：真实地图/地图',
...
CREATE TABLE `member_attendance_tbl` (
  ...
  `status` int(11) DEFAULT NULL COMMENT '打卡状态：出勤,缺勤,未打卡',
  `projectid` varchar(8) DEFAULT NULL COMMENT '项目编号',
...
CREATE TABLE `notice_tbl` (
  ...
  `status` int(11) DEFAULT NULL COMMENT '消息状态：未读,已读',
...
CREATE TABLE `ns_tbl` (
  ...
  `status` int(11) DEFAULT NULL COMMENT '状态：online,offline',
...
CREATE TABLE `point_near_tbl` (
  ...
  `neartype` int(11) DEFAULT NULL COMMENT '信标：Yes,No',
...
CREATE TABLE `project_tbl` (
  ...
  `type` int(11) DEFAULT NULL COMMENT '项目定位类型（正向,反向）',
  `projectype` int(11) DEFAULT NULL COMMENT '定位场景',
  `tenantid` char(8) DEFAULT NULL COMMENT '所属公司',
  ...
  `flag` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '镶嵌功能开启状态',
...
CREATE TABLE `task_management_tbl` (
  ...
  `repetition` int(11) DEFAULT NULL COMMENT '不重复,每天,每月,每周,每一小时,每两小时,每三小时,每四小时,每五小时,每六小时',
  ...
  `begintime` varchar(8) DEFAULT NULL COMMENT '开始时间',
  `endtime` varchar(8) DEFAULT NULL COMMENT '结束时间',
  `states` int(11) DEFAULT NULL COMMENT '任务状态:1.正常2.终止',
...
CREATE TABLE `warning_config_tbl` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `projectid` char(8) NOT NULL DEFAULT '0' COMMENT '项目编号',
  `type` int(11) DEFAULT NULL COMMENT '告警类型：滞留告警,聚集告警,异常静止,越界告警',
  `postype` int(11) DEFAULT NULL COMMENT 'Ble,GPS',
  `meth` int(1) NOT NULL DEFAULT '0' COMMENT '越界告警触发方式1为进，2为出',
  `cate` int(11) DEFAULT NULL COMMENT '类型：人员,车辆,资产',
...
CREATE TABLE `warning_voice_tbl` (
  ...
  `type` int(11) DEFAULT NULL COMMENT '告警类型：SOS告警,聚集告警,越界告警,滞留告警,异常静止,倾斜告警,倾倒告警,火灾告警',
